...
rpm_booklet_bp = Blueprint("rpm_booklet", __name__)  # <-- CHANGE
...
PAGES_DIR = BASE_DIR / "OJT" / "rpm_booklet" / "pages"  # <-- CHANGE
...
@rpm_booklet_bp.route("/rpm")  # <-- CHANGE
def rpm_index():               # <-- CHANGE
    return redirect(url_for("rpm_booklet.rpm_page", page=1))  # <-- CHANGE


# ✅ CHANGE 4: URL prefix + function name
# Example route: "/yxx/<int:page>"
# Example function: yxx_page(page)
@rpm_booklet_bp.route("/rpm/<int:page>")  # <-- CHANGE
def yss_page(page: int):                  # <-- CHANGE
    ...
    return render_template(
        ...
        booklet_title="RP Booklet(Mentor)",  # <-- CHANGE to "NEW Booklet"
        ...
        booklet_endpoint="rpm_booklet.rpm_page"  # <-- CHANGE
    )
