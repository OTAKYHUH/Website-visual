...
nonshift = Blueprint('nonshift', __name__, url_prefix='/nonshift')
DATA_PATH = (Path(__file__).resolve().parents[1] / "data" / "Data.xlsx")

@nonshift.route('/', methods=["GET", "POST"])
def index():
    ...
    shifts = ["D","G"]  # or ["D","N"] if night is "N"

    selected_date = request.form.get("date_filter") or str(date.today())
    ...
    if not filtered_df.empty:
        if selected_shift == "D":
            shift_times = ["07:30","08:30","09:30","10:30","11:30","12:30",
                           "13:30","14:30","15:30","16:30","17:30","18:30"]
        elif selected_shift == "N":
            shift_times = ["19:30","20:30","21:30","22:30","23:30",
                           "00:30","01:30","02:30","03:30","04:30","05:30","06:30"]
        else:
            shift_times = sorted(filtered_df['Time'].unique())

        filtered_df["Time"] = pd.Categorical(filtered_df["Time"],
                                             categories=shift_times,
                                             ordered=True)
        filtered_df = filtered_df.sort_values("Time")

        fig = px.line(
            filtered_df, x="Time", y="YWT",
            category_orders={"Time": shift_times},
            markers=True,
            title=f"YWT vs Time {selected_shift or 'All'}"
        )
        fig.add_shape(type="line", x0=filtered_df["Time"].min(), x1=filtered_df["Time"].max(),
                      y0=9.7, y1=9.7, line=dict(color="red", width=2, dash="dash"))
        fig.add_annotation(x=filtered_df["Time"].max(), y=9.7,
                           text="Target YWT = 9.7", showarrow=False, yshift=10,
                           font=dict(color="red"))
    ...
    return render_template(
        'nonshift.html',      # ← use a simple template name
        shifts=shifts,
