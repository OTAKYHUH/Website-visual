...
yss_booklet_bp = Blueprint("yss_booklet", __name__)  # <-- CHANGE
...
PAGES_DIR = BASE_DIR / "OJT" / "yss_booklet" / "pages"  # <-- CHANGE
...
@yss_booklet_bp.route("/yss")  # <-- CHANGE
def yss_index():               # <-- CHANGE
    return redirect(url_for("yss_booklet.yss_page", page=1))  # <-- CHANGE


# ✅ CHANGE 4: URL prefix + function name
# Example route: "/yxx/<int:page>"
# Example function: yxx_page(page)
@yss_booklet_bp.route("/yss/<int:page>")  # <-- CHANGE
def yss_page(page: int):                  # <-- CHANGE
    ...
    max_page = _max_page()
    if max_page == 0:
        abort(404, description="No YSS booklet pages found.")  # (optional) CHANGE TEXT only
    ...
    return render_template(
        ...
        booklet_title="YSS Booklet",  # <-- CHANGE to "NEW Booklet"
        ...
        booklet_endpoint="yss_booklet.yss_page"  # <-- CHANGE
    )
